...
############################################################
match_data <- function(data, disease_name) {
  if (disease_name == "AE") {
    disease_set <- c("NMDA", "LGI1", "GABA", "GAD65", "CASPR2")
  } else {
    disease_set <- disease_name
  }
  
  sub <- data %>%
    filter(Diagnosis %in% c(disease_set, "HC")) %>%
    mutate(label = ifelse(Diagnosis %in% disease_set, 1, 0))
...
filter_test_data <- function(data, disease_name) {
  if (disease_name == "AE") {
    disease_set <- c("NMDA", "LGI1", "GABA", "GAD65", "CASPR2")
  } else {
    disease_set <- disease_name
...
############################################################
diseases <- c(
  "MS","AQP4Pos_NMOSD","MOGAD",
  "NMDA","LGI1","GABA","GAD65","CASPR2",
  "AE"
)
...
results_list <- lapply(diseases, function(d) {
  ...
  all_seed_info <<- rbind(all_seed_info, data.frame(
    ...
    Description = paste(d, "main random seed"), stringsAsFactors = FALSE
  ))
  ...
  if (!is.null(result$seed_records) && length(result$seed_records) > 0) {
    for (step_name in names(result$seed_records)) {
      all_seed_info <<- rbind(all_seed_info, data.frame(
        ...
        Description = paste(d, step_name, "seed"), stringsAsFactors = FALSE
      ))
    }
  }
...
plot_roc_curves <- function(res_list, title_text) {
  ...
  colors <- c("#8b60b5", "#344aaa", "#0094d4", "#C8B3BC", "#e69c24", "#ad1a71", "#3CB371", "#FFA07A", "#20B2AA", "#9370DB")
  ...
  legend_text <- sapply(seq_along(res_list), function(i) {
    ...
    display_name <- case_when(
      d == "AQP4Pos_NMOSD" ~ "NMOSD",
      d == "AE" ~ "AE",
      TRUE ~ d
    )
    paste0(
      display_name, ": AUC=", format(round(res_list[[i]]$auc, 3), nsmall = 3),
...
if (length(res_int_plot) > 0) {
  pdf(file.path(out_dir, "ROC_Internal_Disease_vs_HC.pdf"), width = 7, height = 7)
  plot_roc_curves(res_int_plot, "ROC Curves Internal (Disease vs HC)")
  dev.off()
  
  png(file.path(out_dir, "ROC_Internal_Disease_vs_HC.png"), width = 7, height = 7, units = "in", res = 300)
  plot_roc_curves(res_int_plot, "ROC Curves Internal (Disease vs HC)")
  dev.off()
}

if (length(res_ext_plot) > 0) {
  pdf(file.path(out_dir, "ROC_External_Disease_vs_HC.pdf"), width = 7, height = 7)
  plot_roc_curves(res_ext_plot, "ROC Curves External (Disease vs HC)")
  dev.off()
  
  png(file.path(out_dir, "ROC_External_Disease_vs_HC.png"), width = 7, height = 7, units = "in", res = 300)
  plot_roc_curves(res_ext_plot, "ROC Curves External (Disease vs HC)")
  dev.off()
}
...
############################################################
cat("\n============================================================\n")
cat("Independent internal and external validation completed.\n")
cat("============================================================\n")
cat("External test site(s):", paste(test_sites, collapse = ", "), "\n")
cat("Output directory:", out_dir, "\n")
cat("Generated key files:\n")
cat("  1. ROC_Internal / External_Disease_vs_HC: ROC curves for successful disease classifiers\n")
